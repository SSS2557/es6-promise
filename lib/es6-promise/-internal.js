...
import asap from './asap-microtask';
...
function subscribe(parent, child, onFulfillment, onRejection) {
  ...
  subscribers[length + INZONE] = getCurrentZone();
...
function publish(promise) {
  ...
  for (var i = 0; i < subscribers.length; i += 4) {
    ...
    zone = subscribers[i + INZONE];

    if (child) {
      invokeCallback(settled, child, callback, detail, zone);
    } else {
...
function tryCatch(callback, detail, zone) {
  try {
...
function invokeCallback(settled, promise, callback, detail, zone) {
  var hasCallback = isFunction(callback),
      value, error, succeeded, failed;

  if (hasCallback) {
    value = tryCatch(callback, detail, zone);
